...
def main():
    ...
    for group, sheet, max_dist in GROUPS:
        ...
        if not api_sensors:
            rows.append((group, 0, 0, "no API sensors in DB — run the tests first"))
            continue
        ...
        rows.append((group, matched, len(api_sensors), ""))

    width = max(len(r[0]) for r in rows)
    for group, matched, n_api, note in rows:
        detail = note if note else f"{matched:>3} matched / {n_api:>3} API sensors"
